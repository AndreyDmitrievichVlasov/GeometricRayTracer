...
elseif apertureType==2
 ...
 printf('Gol error: in flatQuad, apertureType=%d should be 1 - rectangular or 2 - circular. Returning 0\n',apertureType);
 quad_=0;
...
%         d=(L+H)/4; % This d is still bigger than min(L/2,H/2), does not eliminate selfintersection
 ...
 if length(aperture)!=2
  printf("Gol error: in flatQuad>initRectAperture, length(aperture)=%d, should be 2 or 3\n",length(aperture));
  TBN=0;
  return;
 end
end
