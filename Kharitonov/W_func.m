...
% Bi(mod(Phi(u,v,f,lam),2*pi));
% val=1i/lam*exp(-(u.^2+v.^2)/2/sigma^2).*exp(1i*pi*Phi(u,v,f,lam)).*exp(difference)./difference;
% val=1i/lam*exp(-(u.^2+v.^2)/2/sigma^2).*exp(1i*pi*Bi(mod(Phi(u,v,f,lam),2*pi))).*exp(difference)./difference;
