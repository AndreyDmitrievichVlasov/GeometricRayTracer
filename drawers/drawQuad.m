function drawQuad(quad_,handle)
%DRAWQUAD Summary of this function goes here
%   Detailed explanation goes here

   if strcmp(version('-release'),'2015b')
        drawQuadIn2015b(quad_,handle)
    else
        drawQuadInOlderVersions(quad_,handle)
    end
    
end

function drawQuadIn2015b(quad_,handle)
    plot3(handle,quad_.XYZ(1,:),quad_.XYZ(2,:),quad_.XYZ(3,:),'k:','lineWidth',1);
    %TBN
    r_0=quad_.position;

    plot3(handle,[r_0(1) r_0(1)+0.5*quad_.L*quad_.TBN(1,1)],...
             [r_0(2) r_0(2)+0.5*quad_.L*quad_.TBN(2,1)],...
             [r_0(3) r_0(3)+0.5*quad_.L*quad_.TBN(3,1)],...
             'r','lineWidth',1);

    plot3(handle,[r_0(1) r_0(1)+0.5*quad_.H*quad_.TBN(1,2)],...
          [r_0(2) r_0(2)+0.5*quad_.H*quad_.TBN(2,2)],...
          [r_0(3) r_0(3)+0.5*quad_.H*quad_.TBN(3,2)],...
          'g','lineWidth',1);

    plot3(handle,[r_0(1) r_0(1)+0.25*(quad_.H+quad_.L)*quad_.TBN(1,3)],...
        ...
        if endWith(elemType,'DG')%strcmp(elemType,'flatDG')||strcmp(elemType,'sphereDG')||strcmp(elemType,'ellipsoidDG')||strcmp(elemType,'paraboloidDG');
            plot3(handle,quad_.extraData.direction(1,:),quad_.extraData.direction(2,:),quad_.extraData.direction(3,:),...
                 'g--','lineWidth',1.75);
        end
...
%            strcmp(elemType,'sphereDG')||strcmp(elemType,'ellipsoidDG')||strcmp(elemType,'paraboloidDG')
     if  ~isempty(quad_.extraData)
            plotCurveIn3D(quad_.extraData.arc_x,quad_.rotationMatrix,quad_.position,0,handle);
            plotCurveIn3D(quad_.extraData.arc_y,quad_.rotationMatrix,quad_.position,0,handle);
            plotCurveIn3D(quad_.extraData.arc_xy,quad_.rotationMatrix,quad_.position,0,handle);
     end
end

function drawQuadInOlderVersions(quad_,hanlde)
    
 
    plot3(handle, quad_.XYZ(1,:),quad_.XYZ(2,:),quad_.XYZ(3,:),'k:','lineSmooth','on','lineWidth',2);
    %TBN
     r_0=quad_.position;

    plot3(hanlde,[r_0(1) r_0(1)+0.5*quad_.L*quad_.TBN(1,1)],...
            [r_0(2) r_0(2)+0.5*quad_.L*quad_.TBN(2,1)],...
            [r_0(3) r_0(3)+0.5*quad_.L*quad_.TBN(3,1)],...
            'r','lineSmooth','on','lineWidth',1);

    plot3(hanlde,[r_0(1) r_0(1)+0.5*quad_.H*quad_.TBN(1,2)],...
          [r_0(2) r_0(2)+0.5*quad_.H*quad_.TBN(2,2)],...
          [r_0(3) r_0(3)+0.5*quad_.H*quad_.TBN(3,2)],...
          'g','lineSmooth','on','lineWidth',1);

    plot3(hanlde,[r_0(1) r_0(1)+0.25*(quad_.H+quad_.L)*quad_.TBN(1,3)],...
          [r_0(2) r_0(2)+0.25*(quad_.H+quad_.L)*quad_.TBN(2,3)],...
          [r_0(3) r_0(3)+0.25*(quad_.H+quad_.L)*quad_.TBN(3,3)],...
...
function plotCurveIn3D(curve_data,orient_matrix,ref_point,smooth_or_not,handle)
...
draw_data=[orient_matrix(1,1)*curve_data(1,:)+orient_matrix(1,2)*curve_data(2,:)+orient_matrix(1,3)*curve_data(3,:)+ref_point(1);
    ...
    if smooth_or_not==1
          plot3(handle,draw_data(1,:),draw_data(2,:),draw_data(3,:),'k','lineSmooth','on','lineWidth',1.75);
    else
          plot3(handle,draw_data(1,:),draw_data(2,:),draw_data(3,:),'k','lineWidth',1.75);
    end
end
