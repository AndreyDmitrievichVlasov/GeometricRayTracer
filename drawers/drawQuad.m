...
function drawQuadInOlderVersions(quad_)
    hold on
    plot3(quad_.XYZ(1,:),quad_.XYZ(2,:),quad_.XYZ(3,:),'k:','lineSmooth','on','lineWidth',1);
    %TBN
     r_0=quad_.position;
...
draw_data=[orient_matrix(1,1)*curve_data(1,:)+orient_matrix(1,2)*curve_data(2,:)+orient_matrix(1,3)*curve_data(3,:)+ref_point(1);
    ...
    if smooth_or_not==1
          plot3(draw_data(1,:),draw_data(2,:),draw_data(3,:),'k','lineSmooth','on','lineWidth',0.75);
    else
          plot3(draw_data(1,:),draw_data(2,:),draw_data(3,:),'k','lineWidth',0.75);
    end
end
