...
%перевод направления лучей из собственной системы координат в 
% 
% T=
% B=
% N=

% систему координат дифракционной решётки
...
%     et=sum(raysIn(:,4:6).*T,2);
%     
%     e_tte= [(raysIn(:,4) - T(:,1).*et),...
%                (raysIn(:,5) - T(:,2).*et),...
%                (raysIn(:,6) - T(:,3).*et)];
...
function vects = ArrayOfMatrixInvMultByArrayOfVectors(T,B,N,vectorsArray)
    vects=zeros(size(vectorsArray));
    vects(:,1) = T(:,1).*vectorsArray(:,1)+T(:,2).*vectorsArray(:,2)+T(:,3).*vectorsArray(:,3);
    vects(:,2) = B(:,1).*vectorsArray(:,1)+B(:,2).*vectorsArray(:,2)+B(:,3).*vectorsArray(:,3);
    vects(:,3) = N(:,1).*vectorsArray(:,1)+N(:,2).*vectorsArray(:,2)+N(:,3).*vectorsArray(:,3);
